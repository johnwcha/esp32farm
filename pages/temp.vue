  ...
  <section class="section">
    <h3 class="subtitle is-6 has-text-grey">
      <b-icon
        ...
        type="is-danger"
      />
      Current temperature: <span class="has-text-danger">
        <client-only>
        <IOdometer
        class="iOdometer"
        :value="curTempNum"
        format="(,ddd).dd" /> </client-only>
         °F
         </span> ({{ ((Number(curTempRead)-32)*5/9).toFixed(1) }} °C) <br>
       <code> | </code>
    ...
    <TempChart :sensorData=sensorData />
  </section>
</template>
...
export default {
  ...
  computed: {
    curTempNum() {
      return Number(this.curTempRead)
    }
  },
  mounted(){
  ...
  methods: {
      ...
      this.unsubscribe = await onSnapshot(q, (snapshot) => {
        if (this.firstLoad) {
          ...
          snapshot.forEach((doc) => {
            ...
            this.curTempRead = doc.data().temp
    ...
    async initRead() {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // console.log('init sensor read: ', doc.data().temp)
      ...
      this.curTempRead = temperature[0]
